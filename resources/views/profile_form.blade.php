    ...
    <div class="col-lg-12">
        <div class="input-group form-group mb-3">
            ...
            <input type="text" class="form-control" placeholder="{{__('Name')}}" name="name"
                   value="{{auth()->guard('author')->user()->name}}" required>
        </div>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
        <div class="input-group form-group mb-3">
            ...
            <input type="email" class="form-control" placeholder="{{__('Email Address')}}" name="email"
                   value="{{auth()->guard('author')->user()->email}}" required>
        </div>
    </div>
</div>
<div class="row">
            ...
            <div class="card card-primary">
                ...
                <div class="card-body p-1">
                    <img class="img-thumbnail"
                         ...
                         {{asset('storage/profile/'.auth()->guard('author')->user()->avatar)}} @else {{asset('storage/img/no-image.png')}} @endif"
                         alt="{{__('Signature')}}">
                </div>
            </div>
        </div>
